...
- `map.js` (`WorldMap`) draws once on load and on resize, then `render(state)` restyles fills, redraws occupation stripes (SVG patterns per occupier and 25% band), nation borders (a TopoJSON mesh between shapes with different owners), the selected nation's outline, and labels. Labels hide or show depending on zoom level. A `rename` action can change a nation's display name and set a session-only `displayName` on a territory; the map draws that text and the hover tooltip uses it (the original 1939 name stays the object key).
